#Packages
{
  ...
  library(MMLN) #for the updated package (whenever I use it)
}
...
#Data cleaning/preparation
{
  ...
  test_X1 <- test_X1[-c(86:88),]
  newp_X1 <- newp_X1[-c(22:23),]
  ...
  #Remove fake guys
  test_W <- test_W[-c(86:88),]
  newp_W <- newp_W[-c(22:23),]
...
test_psirecovery <- newobsrandeff(np_W = test_W, np_X = test_X2[,-14], np_Z = test_Z[1:nrow(test_W),-c(22:23)], randeff = 1, base = 10, iter = 1000, mhiter = 1, mhscale = .75, proposal = "normbeta", sigma_chain = Sigma_samples, phi_chain = Phi_samples, beta_chain = Beta_samples, whichYstart = "mean", whichCOVprior = "weak", oldway = F)
...
#Okamoto
round(colMeans(newp_W_news[[15]]$predw), 1)
bavg(colMeans(newp_W_news[[15]]$predw))
obp(colMeans(newp_W_news[[15]]$predw))
slg(colMeans(newp_W_news[[15]]$predw))
ops(colMeans(newp_W_news[[15]]$predw))
#true_katoh <- c(66, 22, 2, 14, 0, 3, 42, 4, 110, 183) #update with new data after season ends
#bavg(true_katoh)

#Murakami
round(colMeans(newp_W_news[[16]]$predw), 1)
bavg(colMeans(newp_W_news[[16]]$predw))
obp(colMeans(newp_W_news[[16]]$predw))
slg(colMeans(newp_W_news[[16]]$predw))
ops(colMeans(newp_W_news[[16]]$predw))
...
#Do it for Okamoto
meanpostpsi <- colMeans(newp_W_news[[15]]$psisamples)
LIKES <- apply(newp_W_news[[15]]$psisamples, 1, ppsid_like, mu = meanpostpsi, PostPhi)
pct95 <- round(.95*nrow(newp_W_news[[15]]$psisamples))
sortLIKES <- newp_W_news[[15]]$psisamples[order(LIKES, decreasing = T),]
predpsi95like <- sortLIKES[1:pct95,]
predXBpP <- matrix(0, ncol = 9, nrow = pct95)
for(i in 1:pct95){
  predXBpP[i,] <- matrix(unlist(newp_X2[15,-14]), nrow = 1)%*%PostBeta + predpsi95like[i,]
}
...
# Season isn't over yet; once it is, update true_Okamoto (and do it for Murakami too) above and create the plots
df_newp <- tibble(
  stat = c(colnames(test_W), 'AVG', 'OBP', 'SLG', 'OPS'),
  truth =  c(true_murakami, bavg(true_murakami), obp(true_murakami), slg(true_murakami), ops(true_murakami)),
  pred = c(round(colMeans(newp_W_news[[16]]$predw), 1), bavg(colMeans(newp_W_news[[16]]$predw)), obp(colMeans(newp_W_news[[16]]$predw)), slg(colMeans(newp_W_news[[16]]$predw)), ops(colMeans(newp_W_news[[16]]$predw))),
